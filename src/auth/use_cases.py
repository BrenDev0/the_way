...
from src.auth.service import (
    issue_registration_verification_code,
    verify_registration_email_code,
)
from src.core.cache.ports import CacheStore
from src.core.communications.service import create_verification_email, send_email
...
from src.core.sessions.service import create_session, revoke_session_from_token
...
async def send_registration_verification_code(
    ...
    raw_code, expires_at = await issue_registration_verification_code(
        email_hash=email_hash,
        cache_store=cache_store,
        code_hash_fn=hashing_service.deterministic_hash,
    )
    await asyncio.to_thread(send_email, create_verification_email(raw_code, email))
    return expires_at
...
async def register_user(
    ...
    await verify_registration_email_code(
        email_hash=email_hash,
    ...
    return domain_to_user_response(created_user, encryption_service)
...
async def login_user(
    ...
    return domain_to_user_response(user, encryption_service), raw_session_token
...
async def logout_user(
    ...
    revoked = await revoke_session_from_token(
        token=session_token,

...
def test_whitelist_only_names_real_columns():
...
async def test_database_owned_columns_cannot_be_set(field):
    with pytest.raises(ValidationError):
        await adapter.update_by_id(None, uuid4(), {field: "tampered"})


async def test_rejection_names_every_offending_field():
    with pytest.raises(ValidationError) as exc:

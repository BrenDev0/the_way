...
class TestGetUserReadThrough:
    ...
    async def test_repeated_reads_hit_the_database_once(self, cache_store):
        user = make_user()
        db_reads = []
    ...
    async def test_entry_expires_after_five_minutes(self, cache_store):
        user = make_user()
    ...
    async def test_a_missing_user_is_not_cached(self, cache_store):
        user_id = uuid4()
    ...
    async def test_each_user_has_its_own_entry(self, cache_store):
        a, b = make_user(), make_user()
        assert cache_key(a.id) != cache_key(b.id)


class TestEviction:
    async def test_evict_removes_the_entry(self, cache_store):
        user = make_user()
        cache_store.data[cache_key(user.id)] = mapper.domain_to_cache(user)
    ...
    async def test_evicting_an_uncached_user_is_harmless(self, cache_store):
        await users_use_cases.evict_user(uuid4(), cache_store)

    async def test_a_read_after_eviction_sees_the_new_role(self, cache_store):
...
class TestCacheSerialisation:
    ...
    def test_role_survives_as_an_enum_not_a_string(self):
        user = make_user(role=Role.OWNER)

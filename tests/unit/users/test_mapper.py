...
class TestUserResponseMapper:
    def test_decrypts_email_and_carries_identifiers(self):
    ...
    def test_encrypted_email_never_leaks_into_the_response(self):
        user = make_user(encrypted_email="enc::someone@example.com")
        response = users_mapper.domain_to_user_response(user, FakeEncryptionService())
        assert not response.email.startswith("enc::")


class TestUserSqlAlchemyMapper:
    def test_row_to_domain_maps_renamed_columns(self):
        row = UserRow(
            id=uuid4(),
    ...
    def test_domain_create_to_row_maps_renamed_columns(self):
        create = UserCreate(
            organization_id=uuid4(),

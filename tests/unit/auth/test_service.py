...
def test_generate_numeric_code_default_length_is_six_digits():
    code = auth_service.generate_numeric_code()
    assert len(code) == auth_service.DEFAULT_VERIFICATION_CODE_LENGTH
    assert code.isdigit()


def test_generate_numeric_code_honours_requested_length():
    assert len(auth_service.generate_numeric_code(4)) == 4


async def issue(cache_store, code: str = "123456"):
...
async def test_issue_verification_code_stores_hashed_code_and_zeroed_attempts(cache_store):
    raw_code, expires_at = await issue(cache_store)
...
async def test_issue_verification_code_raw_code_is_never_stored(cache_store):
    raw_code, _ = await issue(cache_store)
    assert raw_code not in cache_store.data.values()


async def test_issue_verification_code_first_request_sets_ttl_on_the_request_counter(cache_store):
    await issue(cache_store)
...
async def test_issue_verification_code_active_cooldown_is_rejected(cache_store):
...
async def test_issue_verification_code_exceeding_max_requests_starts_a_cooldown(cache_store):
...
async def test_issue_verification_code_requests_up_to_the_limit_are_allowed(cache_store):
    cache_store.data[key(AuthCacheKey.VERIFICATION_REQUESTS)] = (
...
async def test_issue_verification_code_code_store_failure_raises(cache_store):
...
async def test_issue_verification_code_attempt_store_failure_rolls_back_the_code(cache_store):
...
async def verify(cache_store, submitted: str):
...
async def test_verify_verification_code_missing_code_is_treated_as_expired(cache_store):
    with pytest.raises(AuthenticationError) as exc:
        await verify(cache_store, "123456")
    assert exc.value.code == "verification_code_expired"


async def test_verify_verification_code_correct_code_clears_all_verification_state(cache_store):
...
async def test_verify_verification_code_wrong_code_increments_attempts(cache_store):
    cache_store.data[key(AuthCacheKey.VERIFICATION_CODE)] = hash_code("123456")
    cache_store.data[key(AuthCacheKey.VERIFICATION_ATTEMPTS)] = 0

    with pytest.raises(AuthenticationError) as exc:
        await verify(cache_store, "000000")
...
async def test_verify_verification_code_final_wrong_attempt_burns_the_code_and_starts_cooldown(cache_store):
    cache_store.data[key(AuthCacheKey.VERIFICATION_CODE)] = hash_code("123456")
